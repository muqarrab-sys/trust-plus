      ...
      <>
        ...
        <View style={{width: 60, height: 60, justifyContent: 'center', alignItems: 'center'}}>
          {this.state.selected[this.state.key] === item[this.state.key] && (
            <View style={{width: 16, height: 16, backgroundColor: '#f5bd00', borderColor: 'gray', borderRadius: 32}}></View>
          )}
        </View>
      </>
    );
    ...
    return (
      <Animated.View
        ...
        <View style={styles.content}>
          <CommonFlatList data={this.state.filteredData} keyExtractor={item => item[this.state.key]} renderItem={item => this.renderItem(item)} />
        </View>
      </Animated.View>
    );
  }
