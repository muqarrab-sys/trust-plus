...
export default function AddWalletStep2Screen({navigation, route}) {
  ...
  const renderMnemonic = (mnemonic, index) => (
    <View style={[styles.mnemonic, {backgroundColor: theme.backgroundColor1}]} key={index}>
      <View style={{width: '80%'}}>
        <CommonText style={{textAlign: 'left', fontWeight: 'bold'}}>
          {index + 1}. {mnemonic}
        </CommonText>
      </View>
    </View>
  ...
  const renderBody = () => {
    return <View style={styles.mnemonicsContainer}>{mnemonics && mnemonics.map(renderMnemonic)}</View>;
  };
  return (
    <Root>
          ...
          <View style={styles.buttonContainer}>
            <View style={styles.row}>
              <CommonButton
                label={lang.continue}
                onPress={() => {
                  navigation.navigate('AddWalletStep3Screen', {mnemonics, item});
                }}
              />
            </View>
          </View>
