...
export default function SmartChainAssetSendScreen({navigation,route}){
    ...
    const onCalculateFee = async ({recipientAddress,amount}) => {
        ...
        if(requestAmount < activeAsset.balance.val && serviceFee < activeWallet.balance.val){
            setValid(true);
        }else{
            setValid(false);
        }
    ...
    const onSubmit = async ({recipientAddress, amount}) => {
        if(valid){
            navigation.navigate('SmartChainAssetConfirmationScreen',{recipientAddress, amount, fee});
        }else{
            CommonToast.error({
                title: lang.error,
    ...
    return (
        <Root>
            ...
            <View style={[styles.header,{backgroundColor: theme.mainColor}]}>
                ...
                <CommonText style={styles.title}>{lang.send} {activeAsset.symbol}</CommonText>
                ...
                <Controller
                    ...
                    render={({onChange, onBlur, value}) => (
                        <WalletAddressTextInput
                            ...
                            onScanPress = {() => {
                                navigation.navigate("SmartChainAssetScannerScreen",{
                                    screenName : "SmartChainAssetSendScreen",
                                    onCallback : (data) => {
