...
export default function PancakeswapScreen({wallet}) {
  ...
  const init = async () => {
    setFromToken(await PancakeswapModule.commonToken(wallet.wallet));
    setToToken({balance: {}});
    setToValue('');
    setTrade([]);
  };
  const reset = async () => {
    ...
    token.balance = await PancakeswapModule.tokenBalance(wallet.wallet, fromToken);
    setFromToken(token);
    setFromValue('');
    //reset toToken
    const token2 = {...toToken};
    token2.balance = await PancakeswapModule.tokenBalance(wallet.wallet, token2);
    setToToken(token2);
    setToValue('');
    //reset Trade
    setTrade([]);
  ...
  const exchange = async (from, to, value) => {
    if (isValidTrade(value)) {
      setError('Loading...');
      const finalValue = value;
      const exchange = await PancakeswapModule.trade(wallet.wallet.address, from, to, finalValue);
  ...
  const swap = async () => {
    CommonLoading.show();
    const result = await PancakeswapModule.swap(wallet.wallet, trade);
  ...
  return (
    <SafeAreaView style={[styles.container, {backgroundColor: theme.backgroundColor1}]}>
      <CommonHeader onPressBack={() => navigation.goBack()} title={language.pancakeswap} textAlign="flex-start" />
      <View style={styles.contentContainer}>
                ...
                onPress={() => {
                  navigation.navigate('BEP20TokenScreen', {
                    onSelect: async token => {
                      const balance = await PancakeswapModule.tokenBalance(wallet.wallet, token);
        ...
        <View style={[styles.exchangeRate, {backgroundColor: theme.backgroundColor3}]}>
          <CommonText>
            {language.pancakeswap} {trade.uniswapVersion}
          </CommonText>
