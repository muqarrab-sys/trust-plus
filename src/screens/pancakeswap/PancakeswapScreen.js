...
export default function PancakeswapScreen({}){
    ...
    const init = async () => {
        setFromToken(await PancakeswapModule.commonToken(activeWallet.wallet));
        setToToken({balance : {}})
        setToValue('');
        setTrade([]);
    }
    const reset = async () => {
        ...
        token.balance = await PancakeswapModule.tokenBalance(activeWallet.wallet, fromToken);
        setFromToken(token);
        setFromValue('');
        //reset toToken
        const token2 = {...toToken};
        token2.balance = await PancakeswapModule.tokenBalance(activeWallet.wallet, token2)
        setToToken(token2);
        setToValue('');
        //reset Trade
        setTrade([]);
        ...
        // dispatch(AssetAction.list(activeNetwork.exchangeId,activeWallet.address));
    }
    ...
    const exchange = async (from,to,value) => {
        if (isValidTrade(value)) {
            setError('Loading...')
            const finalValue = value;
            const exchange = await PancakeswapModule.trade(activeWallet.wallet.address, from, to, finalValue);
    ...
    const swap = async () => {
        CommonLoading.show();
        const result = await PancakeswapModule.swap(activeWallet.wallet, trade);
    ...
    return (
        <SafeAreaView style={[styles.container,{backgroundColor: theme.backgroundColor1}]}>
            <View style={[styles.header,{backgroundColor: theme.mainColor}]}>
                ...
                <CommonText style={styles.title}>{language.pancakeswap}</CommonText>
                <View style={{width : 40}}>
            ...
            <View style={styles.contentContainer}>
                <View>
                    <View style={[styles.row,{backgroundColor : theme.backgroundColor3}]}>
                        <View style={[styles.half,{flex:2}]}>
                            <CommonTouchableOpacity style={[styles.coin,{backgroundColor : theme.backgroundColor3}]} onPress={()=>{
                                navigation.navigate("BEP20TokenScreen",{
                                    onSelect : async (token) => {
                                        const balance = await PancakeswapModule.tokenBalance(activeWallet.wallet, token);
                            ...
                            <View style={[styles.amount,{flex:2,flexDirection : 'row', alignItems : 'center'}]}>
                                <View>
                                    <NumberFormat
                                        ...
                                        decimalScale={4}
                                        renderText={value => (
                                            <CommonText>{language.balance}: {value}</CommonText>
                                        )}
                                    />
                                </View>
                                <View style={[styles.half,{paddingLeft: 5}]}>
                                    <CommonText>{fromToken.symbol}</CommonText>
                                    <CommonTouchableOpacity onPress={async () => {
                    ...
                    <CommonTouchableOpacity style={[styles.exchangeButton,{borderColor : theme.backgroundColor3}]} onPress={async () => {
                        await swapPosition();
                    }}>
                        <Exchange/>
                    </CommonTouchableOpacity>
                ...
                <View style={[styles.exchangeRate,{backgroundColor : theme.backgroundColor3}]}>
                    <CommonText>{language.pancakeswap} {trade.uniswapVersion}</CommonText>
                    <View style={{alignItems : 'center', justifyContent : 'space-between', flexDirection : 'row'}}>
                        {
                            rate > 0 && (
                                <NumberFormat
                                    ...
                                    decimalScale={4}
                                    renderText={value => (
                                        <CommonText>1 {fromToken.symbol} ~ {value} {toToken.symbol}</CommonText>
                                    )}
                                />
                            )
                        }
