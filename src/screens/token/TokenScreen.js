...
export default function TokenScreen({navigation, route}) {
  ...
  useEffect(() => {
    dispatch(TokenAction.getTokens(chainId)).then(data => {
      setLoading(false);
      setFilteredTokens(_.take(data, 15));
    });
  }, []);
  const renderItem = ({item}) => {
    const asset = _.find(assets, function (o) {
      return o.address == item.address;
    });
    ...
    return (
      <View style={styles.item}>
        ...
        <CommonTouchableOpacity
          style={{flex: 1, justifyContent: 'center', alignItems: 'flex-end', paddingRight: 5}}
          onPress={() => {
            CommonLoading.show();
            if (enable) {
              dispatch(AssetAction.removeAsset(chainId, activeWallet, item)).then(() => {
                CommonLoading.hide();
              });
            } else {
              dispatch(AssetAction.addAsset(chainId, activeWallet, item)).then(() => {
                CommonLoading.hide();
              });
            }
          }}>
          <TokenSwitcher enable={enable} />
          <View style={{position: 'absolute', width: 60, height: 40, backgroundColor: `rgba(0,0,0,0.0)`}}></View>
        </CommonTouchableOpacity>
      </View>
    );
  };
