...
export default function EthereumTransactionScreen({navigation, route}) {
  ...
  const renderItem = ({item}) => {
    return (
      <CommonTouchableOpacity
        ...
        onPress={async () => {
          navigation.navigate('EthereumTransactionDetailScreen', {
            item: item,
          });
        }}>
        ...
        <View style={styles.transactionInformation}>
          <View style={{flex: 1}}>
            ...
            {item.status === '0' && <CommonImage source={require('@assets/transaction/loading.gif')} style={{width: 24, height: 24}} />}
            <CommonText>{item.status === '0' ? lang.processing : item.status === '1' ? lang.confirmed : lang.cancelled}</CommonText>
          </View>
          <CommonNumber style={{fontSize: 14}} value={item.etherValue} symbol={activeWallet.network.symbol} />
        </View>
      </CommonTouchableOpacity>
    );
  };
