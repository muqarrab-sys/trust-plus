...
export default function EthereumAssetConfirmationScreen({navigation,route}){
    ...
    const sendERC20 = async () => {
        ...
        if (serviceFeeResult.success) {
            const success = await EtherWalletModule.sendAsset(activeWallet.wallet,recipientAddress,activeAsset.address,fee.requestAmount);
            CommonLoading.hide();
            if (success) {
