...
export default function UniswapScreen(){
    ...
    const init = async () => {
        setFromToken(await UniswapModule.commonToken(activeWallet.wallet));
        setToToken({balance : {}})
        setToValue('');
        setTrade([]);
    }
    ...
    const reset = async () => {
        ...
        token.balance = await UniswapModule.tokenBalance(activeWallet.wallet, fromToken);
        setFromToken(token);
        setFromValue('');
        //reset toToken
        const token2 = {...toToken};
        token2.balance = await UniswapModule.tokenBalance(activeWallet.wallet, token2)
        setToToken(token2);
        setToValue('');
        //reset Trade
        setTrade([]);
    ...
    const swap = async () => {
        CommonLoading.show();
        const result = await UniswapModule.swap(activeWallet.wallet, trade);
    ...
    return (
        <SafeAreaView style={[styles.container,{backgroundColor: theme.backgroundColor1}]}>
            <View style={[styles.header,{backgroundColor: theme.mainColor}]}>
                ...
                <CommonText style={styles.title}>{language.uniswap}</CommonText>
                <View style={{width : 40}}>
                            ...
                            <CommonTouchableOpacity style={[styles.coin,{backgroundColor : theme.backgroundColor3}]} onPress={()=>{
                                navigation.navigate("ERC20TokenScreen",{
                                    onSelect : async (token) => {
                                        const balance = await UniswapModule.tokenBalance(activeWallet.wallet, token);
