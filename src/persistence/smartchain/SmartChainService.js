...
async function getTransactions(address) {
  const network = applicationProperties.networks[2];
  const {status, message, result} = await BscEtherAPI.get(
    'api?module=account&action=txlist&address=' + address + '&startblock=0&endblock=99999999&page=1&offset=10&sort=desc&apikey=' + network.apiKey,
  );
  ...
  data.map(transaction => {
    ...
    transaction.icon =
      transaction.from.toUpperCase() == address.toUpperCase() ? require('@assets/transaction/send.png') : require('@assets/transaction/receive.png');
...
async function getTokenTransactions(address, tokenAddress) {
  ...
  const {status, message, result} = await BscEtherAPI.get(
    'api?module=account&action=tokentx&contractaddress=' +
      ...
      '&address=' +
      address +
      '&page=1&offset=100&startblock=0&endblock=27025780&sort=desc&apikey' +
      network.apiKey,
  );
  const data = status === '1' ? result : [];
  data.map(transaction => {
