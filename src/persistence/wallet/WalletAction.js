...
import {
    addWalletSuccess, getTokenTransactionsSuccess,
...
function setActiveWallet(wallet) {
    return async dispatch => {
        if(wallet.network.symbol === 'BTC'){
            const newWallet = {
                ...wallet,
                wallet : await BitcoinWalletModule.payToWitnessPublicKeyHashMnemonic(wallet),
                balance : await BitcoinWalletModule.getBalance(wallet.address)
            };
            ...
            const newWallet = {
                ...wallet,
                wallet : await EtherWalletModule.fromPrivateKey(wallet.privateKey),
                balance : await EtherWalletModule.getBalance(wallet.address)
            };
            dispatch(setActiveWalletSuccess(newWallet));
            ...
            const newWallet = {
                ...wallet,
                wallet : await BscEtherWalletModule.fromPrivateKey(wallet.privateKey),
                balance : await BscEtherWalletModule.getBalance(wallet.address)
            };
            await WalletService.setActiveWallet(newWallet);
            dispatch(setActiveWalletSuccess(newWallet));
        }
