...
const getTransactions = async (address, last_seen_txid) => {
  const confirmed = await BitcoinAPI.get(`address/${address}/txs/chain/${last_seen_txid ? '' : last_seen_txid}`);
  const unconfirmed = await BitcoinAPI.get(`address/${address}/txs/mempool`);
...
const getFee = async params => {
  ...
  const utxo = await BitcoinModuleService.getUtxo(address);
  let totalAmount = 0;
  ...
  if (change >= 0) {
    const alice = bitcoin.ECPair.fromWIF(wif, network.value);
    let splitAmount = totalAmount;
    let numberOfInputs = 0;
    for (let i = 0; i < utxo.length; i++) {
      const nonWitnessUtxo = await BitcoinModuleService.getTx(utxo[i].txid);
      psbt.addInput({
        hash: utxo[i].txid,
        index: utxo[i].vout,
...
const send = async params => {
  ...
  if (change >= 0) {
    ...
    return await BitcoinModuleService.broadcast(tx);
  }
  return undefined;
};
const BitcoinModuleService = {
  getInfo,
  getUtxo,
  getTx,
  broadcast,
...
export default BitcoinModuleService;
