  ...
  try {
    ...
    try {
      ...
      const btcWallet = {
        ...
        symbol: 'BTC',
        address: btc.address,
        chainId: NETWORKS.btcNetwork.chainId,
        formattedAddress: formatWalletAddress(btc.address),
        balance: {val: 0.0, fiat: 0.0},
        logoURI: 'https://cryptologos.cc/logos/bitcoin-btc-logo.png?v=014',
        privateKey: btc.wif,
        wallet: btc,
        network: NETWORKS.btcNetwork,
      };
      balance += btcWallet.balance.fiat;
      wallets.push(btcWallet);
    } catch (error) {
      console.error("Couldn't create BTC wallet", error);
    }

    // Try to create ETH wallet
    try {
      const ethWallet = {
        id: uuid.v4(),
        name: 'Ethereum',
        symbol: 'ETH',
        chainId: NETWORKS.ethNetwork.chainId,
        ...
        wallet: eWallet,
        network: NETWORKS.ethNetwork,
      };
      balance += ethWallet.balance.fiat;
      wallets.push(ethWallet);
    } catch (error) {
      console.error("Couldn't create ETH wallet", error);
    }

    // Try to create BSC wallet
    try {
      const bWallet = await BscEtherWalletModule.fromPrivateKey(eWallet.privateKey);
      const bscWallet = {
        id: uuid.v4(),
        name: 'Binance Smart Coin',
        symbol: 'BNB',
        chainId: NETWORKS.bscNetwork.chainId,
        ...
        wallet: bWallet,
        network: NETWORKS.bscNetwork,
      };
      balance += bscWallet.balance.fiat;
      wallets.push(bscWallet);
    } catch (error) {
      console.error("Couldn't create BSC wallet", error);
    }

    console.info(`${wallets.length} wallets created: ${wallets.map(w => w.symbol).join(', ')}`);
    return {...params, balance, wallets};
  } catch (e) {
...
async function fromPrivateKey(multiCoinWallet) {
  try {
    ...
    for (var i = 0; i < wallets.length; i++) {
      if (wallets[i].chainId == NETWORKS.btcNetwork.chainId) {
      ...
      } else if (wallets[i].chainId == NETWORKS.ethNetwork.chainId) {
        ...
        wallets[i].transactions = [];
        total += wallets[i].balance.fiat;
      } else if (wallets[i].chainId == NETWORKS.bscNetwork.chainId) {
...
async function reloadWallet(wallet, walletId) {
  try {
    ...
    } else if (updatedWallet.chainId == NETWORKS.ethNetwork.chainId) {
    ...
    } else if (updatedWallet.chainId == NETWORKS.bscNetwork.chainId) {
