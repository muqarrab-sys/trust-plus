import {TokenFactoryPublic, TokensFactoryPublic, TradeContext, PancakeswapPair, PancakeswapPairSettings, BNB} from 'simple-pancakeswap-sdk';
import {BigNumber, utils} from 'ethers';
import _ from 'lodash';
import BscEtherUtilModule from '@modules/bscetherjs/BscEtherUtilModule';
...
const tokenBalance = async (wallet, token) => {
  try {
    if (!_.isNil(token.isBNB)) {
      ...
      return {
        hex: hex,
        val: BscEtherUtilModule.formatEther(BigNumber.from(hex).toString()),
      };
    } else {
      const tokenFactoryPublic = new TokenFactoryPublic(
        ...
        applicationProperties.networks[2].rpcUrl,
      );
      const ethereumAddress = wallet.address;
      const hex = await tokenFactoryPublic.balanceOf(ethereumAddress);
      const formatEther = BscEtherUtilModule.formatEther(BigNumber.from(hex).toString());
      return {
        hex: hex,
        val: formatEther,
      };
...
const getToken = async tokenAddress => {
  try {
    const network = await BscEtherProviderModule.getNetwork();
...
const getTokenBalance = async (wallet, token) => {
  const cryptoBalance = await tokenBalance(wallet, token);
  const fiatBalance = await CurrencyUtil.convert(cryptoBalance.val, token.symbol, 'USD');
  return {
    val: cryptoBalance.val,
    fiat: fiatBalance,
  };
};
