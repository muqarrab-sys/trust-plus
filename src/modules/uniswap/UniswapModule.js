...
async function trade(ethAddress,from,to,value,direction) {
    try{
        const provider = EtherProviderModule.getProvider();
...
const tokenBalance = async (wallet, token) => {
    try{
        const network = await EtherProviderModule.getNetwork();
        ...
        const formatEther = ethers.utils.formatUnits(BigNumber.from(hex),token.decimals);
        return {
            hex : hex,
            val : formatEther
        };
...
const commonToken = async (wallet) => {
    ...
    ethWallet.balance = await tokenBalance(wallet,{address : ethWallet.contractAddress});
